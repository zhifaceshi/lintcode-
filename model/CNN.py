...
class Cnn(nn.Module):
    def __init__(self,vocab_size,embedding_dim,n_filters,filter_sizes,target,vector=None,padding_idx=0):
        ...
        self.convs=torch.nn.ModuleList([torch.nn.Conv2d(in_channels=1,out_channels=n_filters,kernel_size=(fs,embedding_dim),stride=1) for fs in filter_sizes] )
...
class CnnModel(BaseModel):
    def __init__(self,vocab_size,embedding_dim,target_size=2,vector=None,padding_idx=0):
        ...
        self.model=Cnn(vocab_size,embedding_dim,n_filters,filter_sizes,target_size,vector=vector,padding_idx=padding_idx)
