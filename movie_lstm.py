...
if __name__ == '__main__':
    ...
    nonbertdata=NonBertData(train_pth,train_fields,test_pth,test_fields,TEXT,LABEL,x_var,y_vars,x_test_var,None)
    train_iter,test_iter=nonbertdata.getdata()

    
    model=LstmModel(len(TEXT.vocab),100,2,vector=TEXT.vocab.vectors,padding_idx=1)
    optimizer=torch.optim.Adam(model.parameters(),lr=3e-3)
    
    
    train_test=False
    
    modelname="./models_pkl/LSTM_MODEL.pkl"
    if train_test:
        ...
        retdataframe.to_csv('./tmp/LSTM_movie.csv', index=None)
